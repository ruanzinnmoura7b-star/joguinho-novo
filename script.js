// ═══════════════════════════════════════════════════════════
//  Joguinho da Raynara ❤️  — script.js
// ═══════════════════════════════════════════════════════════
...
// ── GRADIENTE SVG (compartilhado nos rings) ───────────────
(function injectSvgDefs() {
  ...
  svg.innerHTML = `<defs>
    <linearGradient id="ringGrad" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#c48fff"/>
...
function startGame() {
  clearEverything();
  score = 0; timeLeft = 30; phase = 1; phaseEnded = false;
  passwordAttempts = 0; holdProgress = 0;
  patienceTrolled = false; finalIndex = 0;
  startPhase();
}
...
const PHASES = {
  1: {
    label: "Fase 1 de 2", title: "Pega os corações",
    tip: "Toque nos corações. Bomba tira pontos. Parece simples.",
    duration: 30, goal: 100,
    items: [
      { emoji: "❤️",  pts: 10, size: 48, speed: () => rand(70,130) },
      { emoji: "💖",  pts: 15, size: 52, speed: () => rand(80,140) },
      { emoji: "💘",  pts: 20, size: 44, speed: () => rand(90,150) },
      { emoji: "💣",  pts:-20, size: 44, speed: () => rand(110,180) }
    ],
    weights: [40, 30, 20, 10],
    spawnMs: 700
  },
  2: {
    label: "Fase 2 de 2", title: "Agora são estrelas",
    tip: "Estrelas valem mais. As bombas continuam, claro.",
    duration: 30, goal: 100,
    items: [
      { emoji: "⭐",  pts: 15, size: 48, speed: () => rand(90,150) },
      { emoji: "✨",  pts: 20, size: 44, speed: () => rand(100,160) },
      { emoji: "🌟",  pts: 25, size: 52, speed: () => rand(80,140) },
      { emoji: "💣",  pts:-20, size: 44, speed: () => rand(130,200) }
    ],
    weights: [35, 30, 20, 15],
    spawnMs: 600
  }
...
function startPhase() {
  ...
  score = 0; timeLeft = cfg.duration; phaseEnded = false;
  canvasItems = [];
  ...
  // Mede o header DEPOIS de mostrar a tela
...
function setupCanvas() {
  ...
  // Posiciona canvas abaixo do header
  canvas.style.top = headerHeight + "px";

  // Escuta toque/clique no canvas
...
function gameLoop(ts) {
  ...
  if (!lastTs) lastTs = ts;
  const dt = Math.min((ts - lastTs) / 1000, 0.1); // segundos
  lastTs = ts;

  const h = canvas.height - headerHeight;
  ...
  for (let i = canvasItems.length - 1; i >= 0; i--) {
    ...
    if (it.hit) {
      ...
      it.scale += dt * 6;
      it.alpha -= dt * 5;
      if (it.alpha <= 0) { canvasItems.splice(i,1); continue; }
    } else {
      it.y += it.vy * dt;
      // Saiu pela base — remove
      if (it.y > h + it.size + 20) { canvasItems.splice(i,1); continue; }
    }
...
function onCanvasTap(e) {
  ...
  for (let t = 0; t < touches.length; t++) {
    ...
    const tx = (touch.clientX || touch.x) - rect.left;
    const ty = (touch.clientY || touch.y) - rect.top;

    // Verifica do topo da lista (itens mais recentes) para baixo
    for (let i = canvasItems.length - 1; i >= 0; i--) {
      const it = canvasItems[i];
      if (it.hit) continue;
      const dx = tx - it.x;
      const dy = ty - it.y;
      const hitR = it.size * 0.75; // raio de hit generoso para mobile
...
function endPhase() {
  ...
  if (phase === 1) {
    ...
    showMessage(
      "👑 Impressionante.",
      "Tá bom, você sabe jogar. Infelizmente pra mim. Vamos testar outra coisa.",
      "Próxima fase",
      startPatienceTest
    );
  }
...
// ── TESTE DE PACIÊNCIA (com anel SVG) ────────────────────
function startPatienceTest() {
  holdProgress = 0; patienceTrolled = false;
  ...
  // Recria o botão para limpar listeners antigos
...
function checkPassword() {
  ...
  if      (passwordAttempts === 1) txt.textContent = "Errada. Mas gostei da confiança.";
  else if (passwordAttempts === 2) txt.textContent = "Ainda não.";
  else if (passwordAttempts === 3) txt.textContent = "Última tentativa. Faz algum sentido agora?";
  else {
...
function restartGame() {
  ...
  patienceTrolled = false; finalIndex = 0;
  document.getElementById("progress").style.width        = "0%";
  document.getElementById("holdProgress") && (document.getElementById("holdProgress").style.width = "0%");
  document.getElementById("loadingProgress") && (document.getElementById("loadingProgress").style.width = "0%");
  showScreen("home");
}
